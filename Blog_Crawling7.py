...
def get_blog(blog_url):
    ...
    try:
        if "naver" in blog_url:
            ...
            # elif soup.select_one("div.lb-l") != None:
            #     category = soup.select_one("div.lb-l").text.replace("\n", "").strip().split(" ")[1].strip()
            #     title = soup.select_one("h1.lb-dm-title").text.replace("\n", "").strip()
            ...
            elif soup.select_one("div#lb_bc") != None:
