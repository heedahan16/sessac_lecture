...
def get_news_list(keyword, start_date, end_date):
    ...
    for date in pd.date_range(start_date, end_date):
        ...
        while True:
            ...
            headers = {"Cookie":"NNB=OURWEDJJKXKGI; nx_ssl=2; _naver_usersession_=+VdPHYAMq5W53YOSu5ttbQ==; page_uid=ieZzndprvmsssdTDFjRssssssah-332673"}
            URL = f"https://search.naver.com/search.naver?where=news&sm=tab_pge&query={keyword}&sort=2&photo=0&field=0&pd=3&ds={start_date}&de={end_date}&mynews=0&office_type=0&office_section_code=0&news_office_checked=&office_category=0&service_area=0&nso=so:r,p:from{startdate}to{enddate},a:all&start={start}"
            res = requests.get(URL, headers=headers)
            soup = BeautifulSoup(res.text, "html.parser")

            if soup.select_one("div.api_noresult_wrap") != None:
                break
    ...
    return pd.DataFrame(news, columns=["제목", "날짜", "매체명", "본문", "URL"])
